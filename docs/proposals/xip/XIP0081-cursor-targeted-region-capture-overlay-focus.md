...
| 2026-06-11 | First-pointer claim replaced with a real-motion gate; per-overlay self-focus gated to the selected overlay; post-map cursor probe added; interactive-capture gate made cross-platform. | The 06-10 first-pointer claim **was itself the regression**: Avalonia raises a synthetic `PointerMoved` on *every* overlay as it opens (log: both monitors fired within 1 ms, +317/+318 ms — impossible for a real cursor), so both overlays claimed and the last one shown (Display 2) always won — "region capture defaults to the second monitor". Fixes: (1) `PointerFocusClaimGate` — claims only on motion beyond a threshold from the first observed position, or on a button press; synthetic events only set the baseline. The gate must track **screen** coordinates with a monitor-bounds filter: COSMIC first maps each overlay on the cursor's output before honoring the requested position, so the same global cursor position produces different *window-local* coordinates across the synthetic burst — fake motion (verified live: Display 2 claimed while the cursor sat on Display 1 until the gate switched to screen coordinates). (2) The watch-item below was confirmed live: `OnOpened` self-focus + retries now run only on the overlay the manager designated (`IsInitialFocusTarget`). (3) `RegionCaptureOptions.CursorPointProvider` lets `OverlayManager` re-read the cursor ~250 ms after mapping — once the XWayland overlays cover every monitor, `xdotool` reads live again — and re-assert the live cursor's overlay even when it matches the initial pick (a transitional claim may have moved focus in between), so targeting is correct even when the user never moves the mouse. (4) `[OverlayFocus]` logging on every decision; today's diagnosis was blind without it. Companion fix: the macOS-only interactive-selector semaphore in `ScreenCaptureService` now applies on all platforms — concurrent overlay sessions (hotkey mashed while one was open) made the new session's grim pre-capture photograph the previous session's frozen dim overlay, which is why captures showed a stale desktop instead of live windows ("screenshot Discord → got the desktop"). |

## Watch-items

- ~~`OverlayWindow.OnOpened` calls `this.Focus()` + `ScheduleDelayedFocusRetries()` on **every** overlay.~~ Confirmed contributing on 2026-06-11 and fixed: self-focus and retries are now gated on `IsInitialFocusTarget`, and all focus transfers route through `OverlayWindow.ClaimActiveWindow()`.
- The post-map cursor probe assumes the Avalonia overlays run as XWayland windows (X11 backend), where a post-map `xdotool` read is live. If the app moves to Avalonia's native Wayland backend, the probe degrades gracefully (provider returns stale/null) but loses its value — revisit alongside XIP0080's native toplevel/workspace support.
